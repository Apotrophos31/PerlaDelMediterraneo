...
<table>
    ...
    <tr>
        <th><%=p.getId()%></th>
        <th><%=p.getNome()%></th>
        <th><%=p.getPrezzo_kg()%></th>
        <th><%=p.getQuantita()%></th>
        <th><%=dateFormat.format(p.getArrivo())%></th>
        <th><%=dateFormat.format(p.getScadenza())%></th>
    </tr>
    <%}%>
...
<form method="post" action="${pageContext.request.contextPath}/prodotti/aggiungi">
    <label>
        Nome:
...
<form method="post" action="${pageContext.request.contextPath}/prodotti/rimuovi">
    <label>
        ID
